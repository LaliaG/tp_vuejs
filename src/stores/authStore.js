...
export const useAuthStore = defineStore('auth', {
    state: () => ({
        user: JSON.parse(localStorage.getItem('user')) || null, // Charger l'utilisateur depuius le localStorage
    }),
    actions: {
        ...
        if (!passwordRegex.test(password)) {
            throw new Error('Le mot de passe doit contenir 8 caractères dont une maj, une min et un chiffre');
        }

        // Enregiste les données dans le localStorage
